...
! !USES:
  ...
  use RtmVar        , only : iulog, barrier_timers, wrmflag, inundflag, sediflag, heatflag, use_dnstrm_boundary   ! Dongyu
...
!-----------------------------------------------------------------------
  ...
  ! model
    ...
    elseif(TUnit%areaTotal2(iunit)/TUnit%rwidth(iunit)/TUnit%rlen(iunit) > 1e6_r8) then ! skip the channel routing if possible numerical instability
       ...
       if ( rtmCTL%mask(iunit) .eq. 3 .and. TUnit%ocn_rof_coupling_ID(iunit) .eq. 0  ) then !If this channel is at basin outlet (downstream is ocean), use the KW method
          call Routing_KW(iunit, nt, theDeltaT)
       else
          if(nt == nt_nliq) then
              if(TRunoff%rslp_energy(iunit) >= TINYVALUE) then ! flow is from current channel to downstream

...
!EOP
  ...
  if (masterproc) then
     ...
     write(iulog,*) 'Successfully read boundary data dimensions'
     call shr_sys_flush(iulog)
  endif
  ...
  if ( .not. found ) call shr_sys_abort( trim(subname)//' ERROR: read data ymd')
  if (masterproc) write(iulog,FORMR) trim(subname),' read data ymd',minval(rtmCTL%ymd_wl),maxval(rtmCTL%ymd_wl)
  call shr_sys_flush(iulog)

  allocate(rtmCTL%tod_wl(rtmCTL%ntime_wl))
  call ncd_io(ncid=ncid, varname='tod', flag='read', data=rtmCTL%tod_wl, readvar=found)
  if ( .not. found ) call shr_sys_abort( trim(subname)//' ERROR: read data tod')
  if (masterproc) write(iulog,FORMR) trim(subname),' read data tod',minval(rtmCTL%tod_wl),maxval(rtmCTL%tod_wl)
  call shr_sys_flush(iulog)

  allocate(rtmCTL%wl(ndata))
  call ncd_io(ncid=ncid, varname='waterlevel', flag='read', data=rtmCTL%wl, readvar=found)
  if ( .not. found ) call shr_sys_abort( trim(subname)//' ERROR: read data wl')

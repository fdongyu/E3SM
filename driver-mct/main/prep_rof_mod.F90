...
#include "shr_assert.h"
  ...
  use seq_comm_mct,     only: num_inst_lnd, num_inst_rof, num_inst_frc, num_inst_atm, num_inst_ocn ! Dongyu
  ...
  use component_type_mod, only: rof, lnd, atm, ocn  ! Dongyu
  ...
  type(seq_map), pointer :: mapper_So2r ! Dongyu
  type(seq_map), pointer :: mapper_Fo2r ! Dongyu Flux

  ! attribute vectors
  type(mct_aVect), pointer :: l2r_rx(:)
  type(mct_aVect), pointer :: a2r_rx(:)
  type(mct_aVect), pointer :: o2r_rx(:) ! Dongyu
  ...
  type(mct_aVect), pointer :: o2racc_ox(:)   ! Dongyu ocn export, ocn grid, cpl pes
  integer        , target  :: o2racc_ox_cnt  ! Dongyu o2racc_ox: number of time samples accumulated
...
contains
  ...
  subroutine prep_rof_init(infodata, lnd_c2_rof, atm_c2_rof, ocn_c2_rof)
    ...
    logical                 , intent(in)    :: ocn_c2_rof ! Dongyu .true.  => ocn to rof coupling on
    !
    ! Local Variables
    integer                     :: lsize_r
    integer                     :: lsize_l
    integer                     :: lsize_a
    integer                     :: lsize_o       ! Dongyu
    integer                     :: eli, eri, eai, eoi ! Dongyu
    logical                     :: samegrid_lr   ! samegrid lnd and rof
    logical                     :: samegrid_ar   ! samegrid atm and rof
    logical                     :: samegrid_ro   ! Dongyu, ro is already defined
    logical                     :: esmf_map_flag ! .true. => use esmf for mapping
    logical                     :: rof_present   ! .true.  => rof is present
    logical                     :: lnd_present   ! .true.  => lnd is present
    logical                     :: atm_present   ! .true.  => atm is present
    logical                     :: ocn_present   ! Dongyu .true.  => ocn is present
    logical                     :: iamroot_CPLID ! .true. => CPLID masterproc
    character(CL)               :: atm_gnam      ! atm grid
    character(CL)               :: lnd_gnam      ! lnd grid
    character(CL)               :: rof_gnam      ! rof grid
    character(CL)               :: ocn_gnam      ! Dongyu ocn grid
    type(mct_aVect) , pointer   :: l2x_lx
    type(mct_aVect) , pointer   :: a2x_ax
    type(mct_aVect) , pointer   :: o2x_ox        ! Dongyu
    ...
    allocate(mapper_So2r) ! Dongyu ocn state mapper
    allocate(mapper_Fo2r) ! Dongyu Flux mapper
    ...
    if (rof_present .and. ocn_present) then
       ...
       if (ocn_c2_rof) then
          ...
          ! Dongyu initialize flux mapper
          call seq_map_init_rcfile(mapper_Fo2r, ocn(1), rof(1), &
               'seq_maps.rc','ocn2rof_fmapname:','ocn2rof_fmaptype:',samegrid_ro, &
  ...
  subroutine prep_rof_accum_avg(timer)
    ...
    !
    ! Local Variables
    integer :: eri, eli, eai, eoi  ! Dongyu
  ...
  subroutine prep_rof_mrg(infodata, fractions_rx, timer_mrg, cime_model)
    ...
    do eri = 1,num_inst_rof
       ...
       call prep_rof_merge(l2r_rx(eri), a2r_rx(eri), o2r_rx(eri), fractions_rx(efi), x2r_rx, cime_model, ocn_present) ! Dongyu
    end do
  ...
  subroutine prep_rof_merge(l2x_r, a2x_r, o2x_r, fractions_r, x2r_r, cime_model, ocn_present)
    ...
    type(mct_aVect),intent(in)    :: o2x_r   ! Dongyu
    type(mct_aVect),intent(in)    :: fractions_r
    type(mct_aVect),intent(inout) :: x2r_r
    character(len=*)        , intent(in)    :: cime_model
    logical                 , intent(in)    :: ocn_present   ! Dongyu .true.  => ocn is present
    !
    ! Local variables
    integer       :: i
    ...
    integer, save :: index_x2r_So_ssh         ! Dongyu
    integer, save :: index_o2x_So_ssh         ! Dongyu
    integer, save :: index_x2r_So_s_ocn2rof    ! Dongyu
    integer, save :: index_o2x_So_s_ocn2rof    ! Dongyu
    !integer, save :: index_x2r_Foro_saltFlux    ! Dongyu
    !integer, save :: index_o2x_Foro_saltFlux    ! Dongyu
    ...
    if (first_time) then
       ...
       if (ocn_present) then
          index_o2x_So_ssh = mct_aVect_indexRA(o2x_r,'So_ssh')    ! Dongyu
          index_x2r_So_ssh = mct_aVect_indexRA(x2r_r,'So_ssh')    ! Dongyu
          mrgstr(index_x2r_So_ssh)       = trim(mrgstr(index_x2r_So_ssh))//' = '//'o2x%So_ssh' ! Dongyu
          index_o2x_So_s_ocn2rof = mct_aVect_indexRA(o2x_r,'So_s_ocn2rof')    ! Dongyu
          index_x2r_So_s_ocn2rof = mct_aVect_indexRA(x2r_r,'So_s_ocn2rof')    ! Dongyu
          mrgstr(index_x2r_So_s_ocn2rof)       = trim(mrgstr(index_x2r_So_s_ocn2rof))//' = '//'o2x%So_s_ocn2rof' ! Dongyu
          !index_o2x_Foro_saltFlux = mct_aVect_indexRA(o2x_r,'Foro_saltFlux') ! Dongyu
          !index_x2r_Foro_saltFlux = mct_aVect_indexRA(x2r_r,'Foro_saltFlux') ! Dongyu
          !mrgstr(index_x2r_Foro_saltFlux)= trim(mrgstr(index_x2r_Foro_saltFlux))//' = '//'o2x%Foro_saltFlux' ! Dongyu
       endif
    ...
    do i = 1,lsize
       ...
       !if ( ocn_present ) then
       !   x2r_r%rAttr(index_x2r_So_ssh,i)        = o2x_r%rAttr(index_o2x_So_ssh,i)          ! Dongyu
       !   x2r_r%rAttr(index_x2r_Foro_saltFlux,i) = o2x_r%rAttr(index_o2x_Foro_saltFlux,i)   ! Dongyu
       !endif

    end do

    ! add this outside the loop, so it works for dlnd compset
    if ( ocn_present ) then
       do i =1,lsize
          x2r_r%rAttr(index_x2r_So_ssh,i)        = o2x_r%rAttr(index_o2x_So_ssh,i)          ! Dongyu
          x2r_r%rAttr(index_x2r_So_s_ocn2rof,i)  = o2x_r%rAttr(index_o2x_So_s_ocn2rof,i)    ! Dongyu
          !x2r_r%rAttr(index_x2r_Foro_saltFlux,i) =  o2x_r%rAttr(index_o2x_Foro_saltFlux,i)  ! Dongyu
       enddo
    endif
  ...
  subroutine prep_rof_calc_o2r_rx(timer)
    ...
    do eri = 1,num_inst_rof
       ...
       !call seq_map_map(mapper_Fo2r, o2racc_ox(eoi), o2r_rx(eri), fldlist=seq_flds_o2x_fluxes_to_rof, norm=.true.)  ! Dongyu no flux added for now
    end do
    call t_drvstopf  (trim(timer))
